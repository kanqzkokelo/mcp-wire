...
var hostCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		node.Host.SetStreamHandler(protoID, func(s network.Stream) {
			...
			if err != nil {
				slog.Warn("host handshake failed", "peer", s.Conn().RemotePeer().String(), "error", err)
				return
			}
...
func init() {
	...
	hostCmd.Flags().StringVar(&authToken, "token", "", "optional pre-shared authorization token secret")
